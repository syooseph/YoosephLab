...
{
    std::fstream fstrm;
    io::openFile( fstrm, graph_input.c_str(), std::ios::in | std::ios::binary );
...
bool graph::hasNode( NodeList &path,
                         NodeType target )
{
...
void graph::dropAncestors( AdjacencyMap &ancestors, 
                               ...
                               PathType &major_path )
{
    // NodeArray major = ListToVector<NodeType>(major_path);
    // NodeArray minor = ListToVector<NodeType>(minor_path);
...
NodeSet graph::trimGraph( DeBruijnGraph &graph,
                          CoverageMap &kmer_coverage,
                          VertexToKmerMap &vertex_map,
                          int min_depth,
                          bool verbose )
                                  ...
                                  size_t min_coverage )
{
    ...
    for ( next = vc; vc != ve; vc = next ) {
        ++next;
//         if ( kmer_coverage[vertex_map[*vc]] < min_coverage ) {

//             clear_vertex(*vc, graph);
...
//         }
        ...
        for ( size_t i = 0; i < preds.size(); i++ ) {
            //if ( deleted.find(preds[i]) != deleted.end() ) continue;
        ...
        //deleted.push_back(*vc);
        //std::cout << "Removing vertex:" << *vc << "\t" << alpha::IntegerToAminoAcid(vertex_map[*vc],6) << "\n";
                                 ...
                                 ) 
{
    ...
    for ( next = vc; vc != ve; vc = next ) {
        ...
        if ( ancestors[*vc].size() == 0 && successors(graph, *vc).size() == 0 ) {
            //std::cout << "Removing island:" << *vc << "\t" << alpha::IntegerToAminoAcid(vertex_map[*vc], 6) << "\n";
 ...
 * \param graph DeBruijnGraph object.
...
// Vertex findVertex(DeBruijnGraph &graph, KmerId kmer) {
// 	Vertex_iter vc, ve;
// 	for ( boost::tie(vc,ve) = vertices(graph); vc != ve; ++vc ) {
// 		if ( kmer == graph[*vc].kmer )
// 			return *vc;
// 	}
// 	return *ve;
// }
...
// findKmer(DeBruijnGraph &graph, KmerId kmer) {
// 	Vertex_iter vc, ve;
// 	for ( boost::tie(vc,ve) = vertices(graph); vc != ve; ++vc ) {
// 		if ( kmer == graph[*vc].kmer )
// 			return std::make_pair(*vc, *ve);
// 	}
// 	return std::make_pair(*ve, *ve);
// }
...
bool graph::isSource(Vertex v,
					 ...
					 AdjacencyMap &ancestors) 
{
    if ( ancestors[v].size() == 0 && out_degree(v,g) > 0 )
        return true;
    return false;
}
 ...
 * \param graph  DeBruijn Graph.
 * \param vertex vertex of interest.
 * \param ancestors ancestor nodes of vertex v.
 * \return True/False.
 */ 
//inline 
