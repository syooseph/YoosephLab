...
namespace seq
{
	...
	inline bool validSequence(SeqType &seq) 
	{
...
/* 	} */
						...
						int mode )
	{
		...
		boost::iostreams::filtering_streambuf<boost::iostreams::input> in;
		//if ( compressed ) 
		if ( io::getFileExtension( std::string(filename) ) == "gz" ) 
			in.push(boost::iostreams::gzip_decompressor());
		in.push(fstrm);
		std::istream istrm(&in);
...
/* 		}  */
		...
		while ( std::getline(istrm, line) ) {
			if ( line[0] == '>' ) {
				...
				fasta_tag = line; fasta_seq = ""; 
				if ( count && count % 1000000 == 0 ) std::cout << "\t" << count << " sequences\n";
			}
			else fasta_seq += line;
		}
		if ( fasta_tag != "" && fasta_seq != "") {
	...
	inline std::set<int> loadIndices( const char *filename ) 
	{
		...
		std::fstream fin;
		io::openFile(fin, filename, std::ios::in);
...
/* 	} */
	...
	//bool zflag )
	{
		...
		readSequences(file.c_str(), tags, seqs, count, mode);//, zflag );
