...
my $inc_reads_folder = $run_folder . '/Reference_BLASTP/';
...
foreach(<$inc_reads_folder/Q92401.*.inc.reads>) {
  my $inc_read_file = $_;
  ...
  if(-e "$grasp_folder/$seq_ID.fa.0.reads" and -e "$blast_folder/$file_stem.blast.summary"
    and -e "$psiblast_folder/$seq_ID.psibst" and -e "$fastm_folder/$file_stem.fastm.summary"
    ...
    while(<$BIN>) {
      ...
      if($decom[1] =~ /$seq_ID/ || $decom[10] > $score_cutoff)  {
        next;
      }
      #$decom[1] =~ /(.*)\:\:(\d+)-(\d+)/;
    ...
    open my $PIN, "<$psiblast_folder/$seq_ID.psibst" or die "Cannot open blast summary file: $!\n";
    while(<$PIN>) {
      chomp;
      if(/^#/ || /^\~/)  {
        next;
    ...
    my %fastm_hash;
    open my $FIN, "<$fastm_folder/$file_stem.fastm.summary" or die "Cannot open grasp summary file: $!\n";
    while(<$FIN>) {
      chomp;
      if(/^\>\>(\S*)\s+\(/)  {
        my $id = $1;
    ...
    while(<$XIN>)  {
      ...
      if($decom2[0] =~ /$decom1[2]/ || $decom2[2] > $score_cutoff)  {
        next;
      }
