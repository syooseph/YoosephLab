  ...
  for(my $j = 0; $j < scalar(@{$array->[0]}); $j += 2)  {
    ...
    for(my $i = 1; $i < $num_rows; ++ $i)  {
      if($array->[$i][$j + 1] < $array->[$i - 1][$j + 1])  {
        ...
        #print "$array->[$i][$j]	$array->[$i][$j + 1]	$array->[$i - 1][$j]	$array->[$i - 1][$j + 1]  $area\n";
      } else  {
        $array->[$i][$j + 1] = $array->[$i - 1][$j + 1];
      }
    }
    # compute the last area by extrapolation, 0.01 is added as pseudo count
    my $slope = ($array->[$num_rows - 1][$j] - $array->[$num_rows - 2][$j]) 
      / (0.01 + $array->[$num_rows - 2][$j + 1] - $array->[$num_rows - 1][$j + 1]); 
    $slope /= 2; # extrapolated by taking the final slope 0
    my $intersect = $array->[$num_rows - 1][$j + 1] * $slope + $array->[$num_rows - 1][$j];
    if($intersect > 1)  {
      $intersect = 1;	# we do not allow sensitivity  > 1
    }
    #print "final nums:  $array->[$num_rows - 1][$j]	$array->[$num_rows - 1][$j + 1]	 $array->[$num_rows - 2][$j]	$array->[$num_rows - 2][$j + 1]\n";
    #print "slope and intersect:	$slope	$intersect\n";
...
for(my $i = $e_min; $i <= $e_max; ++ $i)  {
  ...
  while(<$IN>)  {
    chomp;
    my @decom = split /\s+/, $_;
    if(exists $gid{$decom[0]})  {	# if the gene is presented in the gene sheet
...
my @auc = ComputeAUCFromMatrix(\@performances);
print "@auc\n";
