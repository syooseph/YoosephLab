...
foreach(<$inc_reads_folder/*.inc.reads>) {
  ...
  if(-e "$grasp_folder/$seq_ID.fa.0.reads" and -e "$blast_folder/$file_stem.blast.summary"
    and -e "$psiblast_folder/$file_stem.blast.summary" and -e "$fastm_folder/$file_stem.fastm.summary")  {
    #print "...\n";
    ...
    open my $PIN, "<$psiblast_folder/$file_stem.blast.summary" or die "Cannot open blast summary file: $!\n";
    while(<$PIN>) {
      chomp;
      if(/^#/)  {
        next;
