...
foreach(<$inc_reads_folder/*.ref>) {
  ...
  if(-e "$grasp_folder/$seq_ID.fa.reads" and -e "$blast_folder/$file_stem.frag.bst"
    ...
    and -e "$run_folder/map_results.summary")  {
    #print "...\n";
    ...
    open my $XIN, "<$run_folder/map_results.summary" or die "Cannot open GRASPx summary file: $!\n";
    while(<$XIN>)  {
      chomp;
      my @decom = split /\s+/, $_;
      ...
      if($decom3[0] ne $seq_ID || $decom2[2] > $score_cutoff)  {
        next;
      }
