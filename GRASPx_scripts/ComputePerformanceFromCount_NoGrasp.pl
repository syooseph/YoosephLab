...
while(<$IN>) {
  ...
  if($decom[9] > 0)  {
    if($decom[1] > 0)  {
      $graspx_sen += $decom[8] / $decom[1];
    }  else  {
      $graspx_sen += 1;
    }
    #$grasp_sen += $decom[8] / $decom[1];
    $graspx_spe += $decom[8] / $decom[9];
  }  else  {
    if($decom[1] > 0)  {
      $graspx_sen += $decom[8] / $decom[1];
    }  else  {
...
print "$blast_par_sen  $blast_par_spe  $psiblast_par_sen  $psiblast_par_spe  $fastm_sen  $fastm_spe  $graspx_sen $graspx_spe\n";
