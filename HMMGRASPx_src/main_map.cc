  ...
  */
  ...
  if(write_to_file)  {
    for(auto it = identified_reads.begin(); it != identified_reads.end(); ++ it) {
      out_fh << (int) (it->rid) << " " << db_seq.GetHeader(it->rid) << "  " << re_aln.GetContigInfo(it->contig_id) << " " << it->num_errors << "  " << it->aligned_portion << endl;
    }
    out_fh.close();
  } else  {
    for(auto it = identified_reads.begin(); it != identified_reads.end(); ++ it) {
      cout << (int) (it->rid) << " " << db_seq.GetHeader(it->rid) << "  " << re_aln.GetContigInfo(it->contig_id) << it->num_errors << "  " << it->aligned_portion << endl;
    } 
  }
  return 0;
}
