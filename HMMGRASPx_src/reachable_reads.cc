...
ReachableReads::ReachableReads(
  ...
  db_load.LoadSeedExt(seed_ext_file, seq_obj, seed_ext_);
  db_load.LoadReadExt(read_ext_file, fw_read_ext_, re_read_ext_);
  db_load.LoadHighScore3mer(hsm_file, high_score_match_);
...
int ReachableReads::GetSeedLen(void) {
  return seed_len_;
}
...
void ReachableReads::GetHighScoreSeq(
  ...
  while(index < num_iter) {
    ...
    uint16_t s_idx = db_obj_foo.Convert3mer(s.c_str());
    if(high_score_match_[s_idx].size() <= 0)  {
    ...
    for(auto it_c = seq_temp.begin(); it_c != seq_temp.end(); ++ it_c) {
      for(auto it_ap = high_score_match_[s_idx].begin(); 
          it_ap != high_score_match_[s_idx].end(); ++ it_ap
      ) {
        char s_str[4];
        db_obj_foo.Interpret3merIndex(*it_ap, s_str);
        seq_app[it_c->first + string(s_str)] = true;
      }
    }
    seq_temp = seq_app;
    ++ index;
  ...
  if(num_olp != 0)  {
    ...
    for(auto it_c = seq_temp.begin(); it_c != seq_temp.end(); ++ it_c) {
      for(auto it_ap = high_score_match_[s_idx].begin(); 
        ...
        char s_str[4];
        db_obj_foo.Interpret3merIndex(*it_ap, s_str);
        hs_seq[it_c->first + string(&s_str[num_olp])] = true;
      }
    }
  } else  {
    hs_seq = seq_temp;
...
void ReachableReads::SelectSeeds(
  ...
  RefineSeedDiffScoreHigh(ceil(0.8 * seed_len_), candidate_seeds);
  // DEBUG print
...
// used for sorting SeedType based on their positions
bool _cmp_seed_ext(SeedType &s1, SeedType &s2)  {
  if(s1.num_extension < s2.num_extension)  {
    return true;
  }
  return false;
}

void ReachableReads::RefineSeed(
    int min_overlap, int query_len, 
    std::map<int, std::list<SeedType> > &candidate_seeds
) {
  // sort the seeds based on their potential of extensions, prioritize ones with less extensions
  for(auto it = candidate_seeds.begin(); it != candidate_seeds.end(); ++ it) {
    for(auto it_s = it->second.begin(); it_s != it->second.end(); ++ it_s) {
      it_s->num_extension = seed_ext_[it_s->seed_seq].size();
    }
    it->second.sort(_cmp_seed_ext);
  }
  // get non-redundant high-score seeds one-by-one
  vector<unordered_map<string, bool> > uniq_mers(query_len); // the key is overlap mer
  map<int, std::list<SeedType> > nr_candidate_seeds;
  // for each score
  for(auto it = candidate_seeds.begin(); it != candidate_seeds.end(); ++ it) {
    // for each seed
    for(auto it_s = it->second.begin(); it_s != it->second.end(); ++ it_s) {
      bool is_redundant = false;
      // for each position of the seed
      for(int i = 0; i < seed_len_ - min_overlap; ++ i) {
        if(uniq_mers[it_s->q_pos + i].find(it_s->seed_seq.substr(i, min_overlap)) 
            != uniq_mers[it_s->q_pos + i].end()
        )  {
          // redundancy detected
          is_redundant = true;
          break;
      ...
      if(!is_redundant)  {
        // take the non-redundant seed
        nr_candidate_seeds[it->first].push_back(*it_s);
        // record kmer for redundancy detection
        for(int i = 0; i < seed_len_ - min_overlap; ++ i) {
          uniq_mers[it_s->q_pos + i][it_s->seed_seq.substr(i, min_overlap)] = true;
        }
      }
    }
  }
  candidate_seeds = nr_candidate_seeds;
  return; 
}

// used for sorting SeedType based on their positions
bool _cmp_seed_pos(SeedType &s1, SeedType &s2)  {
  if(s1.q_pos < s2.q_pos)  {
    return true;
  }
  return false;
}
...
void ReachableReads::RefineSeedDiffScoreHigh(
    int min_overlap, 
    std::map<int, std::list<SeedType> >& candidate_seeds
) {
  // note that the first field in the map indicates position in the query
  map<int, list<string> > presented_seeds;
  auto it = candidate_seeds.begin();
  // loop from high-score seeds to low-score seeds
  while(it != candidate_seeds.end()) {
...
void ReachableReads::RefineSeedDiffScoreLow(
    int min_overlap, 
    std::map<int, std::list<SeedType> >& candidate_seeds
) {
  // note that the first field in the map indicates position in the query
...
std::list<OverlapType>::iterator
ReachableReads::GetExtIterBeginFW(RIDType rid) {
  return fw_read_ext_[rid].begin();
}

std::list<OverlapType>::iterator
ReachableReads::GetExtIterBeginRE(RIDType rid) {
  return re_read_ext_[rid].begin();
}
