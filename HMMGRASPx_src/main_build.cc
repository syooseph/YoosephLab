...
int main(int argc, char** argv)  {
  ...
  desc.add_options()
      ...
      ("seed_coverage", boost::program_options::value<int>(&min_seed_coverage)->default_value(2), "minimum coverage required for seed sequence")
      ...
      ("alphabet", boost::program_options::value<int>(&alph_id)->default_value(3), "reduced alphabet to be used for seeding\n  0:ALL20  1:DSSP5  2:DSSP10  3:GBMR4\n  4:GBMR10  5:HSDM5  6:SDM6  7:MURPHY5\n  8:MURPHY10  9:TD5  10:TD10")
  ;
  ...
  std::list<PositionType> fw_seed_ext_new;
  db_dump.GetSeedExtNew(db_seq, min_seed_coverage, fw_seed_ext_new);
  /*
  for(auto it = fw_seed_ext.begin(); it != fw_seed_ext.end(); ++ it) {
    for(auto it_s = it->second.begin(); it_s != it->second.end(); ++ it_s) {
      cout << it->first << "  " << (unsigned int) it_s->rid << " " << (unsigned int) it_s->pos << endl;
    }
  }
  */
  ...
  std::list<PositionType> re_seed_ext_new;
  db_dump.GetSeedExtRevNew(db_seq_rev, min_seed_coverage, re_seed_ext_new);
  
  /*
  for(auto it = re_seed_ext.begin(); it != re_seed_ext.end(); ++ it) {
    for(auto it_s = it->second.begin(); it_s != it->second.end(); ++ it_s) {
      cout << it->first << "  " << (unsigned int) it_s->rid << " " << (unsigned int) it_s->pos << endl;
    }
  }
  */
  /*
  for(auto it = re_seed_ext_new.begin(); it != re_seed_ext_new.end(); ++ it) {
    cout << (unsigned int) it->rid << " " << (unsigned int) it->pos << endl;
  }
  */
  ...
  list<MinReadPairType> seed_pair_ext_new;
  db_dump.MatchSeedPairNew(db_seq, fw_seed_ext_new, re_seed_ext_new, seed_pair_ext_new);
  /*
  for(auto it = seed_pair_ext.begin(); it != seed_pair_ext.end(); ++ it) {
    for(auto it_s = it->second.begin(); it_s != it->second.end(); ++ it_s) {
      cout << it->first << "  " << (unsigned int) it_s->init_fw << " " << (unsigned int) it_s->init_re << endl;
    }
  }
  */
  /*
  for(auto it = seed_pair_ext_new.begin(); it != seed_pair_ext_new.end(); ++ it) {
    cout << (unsigned int) it->init_fw << " " << (unsigned int) it->init_re << endl;
  }
  */
  ...
  db_dump.DumpSeedExt(seed_pair_ext_new, out_file_seed_ext);
  ...
  map<uint16_t, list<uint16_t> > hs_mer;
  db_dump.CreateHighScore3mer(hs_mer);
  string out_file_hs_mer = workspace_dir + "/" + db_stem + ".hsm";
  db_dump.DumpHighScore3mer(hs_mer, out_file_hs_mer);
