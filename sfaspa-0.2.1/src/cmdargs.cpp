                                    ...
                                    Param &param)
{
    ...
    //std::string merge_short_ratio_str = std::to_string( MERGE_SHORT_RATIO );
    std::sprintf(temp, "%.1f", MERGE_SHORT_RATIO);
    std::string merge_short_ratio_str = temp;
    //std::string extend_score_str = std::to_string( EXTEND_SCORE );
    std::sprintf(temp, "%.1f", EXTEND_SCORE);
    std::string extend_score_str = temp;
    //std::string read_align_score_str = std::to_string( READ_ALIGN_SCORE );
    std::sprintf(temp, "%.1f", READ_ALIGN_SCORE);
    std::string read_align_score_str = temp;
    //std::string read_align_ratio_str = std::to_string( READ_ALIGN_RATIO );
    ...
    try {
        ...
        generic.add_options()
            ...
            ("seed-coverage,S", po::value<int>(&param.min_seed)->default_value(MIN_SEED), "Minimum coverage of seed kmer")
            ("read-support,r", po::value<int>(&param.min_share)->default_value(SHARED_READS), "Minimum number of same reads in a neighboring node")
            ("overlap-length,b", po::value<int>(&param.back_trace)->default_value(BACK_TRACE), "Minimum length of string for suffix array search")
        ...
        hidden.add_options()
            ...
            ("trim-graph,T", po::value<bool>(&param.trim_flag)->default_value(TRIM_GRAPH), "Trim low coverage vertices/edges")
            ("min-coverage,c", po::value<int>(&param.min_depth)->default_value(MIN_DEPTH), "Minimum kmer coverage for trimming graph")
            //("uniq-seed,U", po::value<bool>(&param.uniq_seed)->default_value(UNIQ_SEED), "Skip a kmer seed that discovered in another path")
            ("seed-reuse,u", po::value<bool>(&param.seed_reuse)->default_value(SEED_REUSE), "Use seed k-mers found in previous paths for seeding paths")
            ("dump-flag,D", po::value<bool>(&param.dump_flag)->default_value(DUMP_BINARIES), "Dump output as binary")
            ...
            ("skip-fail", po::value<bool>(&param.skip_fail)->default_value(SKIP_FAILED_SEED_FIRST), "Avoid seed kmers from failed paths")
            ("try-later", po::value<bool>(&param.try_later)->default_value(RUN_FAILED_SEED_LATER), "Try failed seed later if skipped")
            ...
            ("ignore-stop", po::value<bool>(&param.ignore_stop)->default_value(IGNORE_STOP_CODON), "Ignore stop codon while path extension")
            ("replace-stop", po::value<bool>(&param.replace_stop)->default_value(REPLACE_STOP_CODON), "Replace stop codon with other amino acid during consensus call, if possible")
            ("clip-stop", po::value<bool>(&param.clip_stop)->default_value(CLIP_AFTER_STOP_CODON), "Clip sequence away after stop codon")
...
void args::parse_cmd_args_poststat( int argc, 
                                    char ** argv, 
                                    size_t      &num_read,
                                    size_t      &min_length,
                                    //std::string &orig_faa,
                                    ...
                                    bool &verbose )
{
    try {
        ...
        po::options_description list("File options");
        list.add_options()
            ("num-read,n", po::value<size_t>(&num_read), "Number of input peptide reads")
            ("min-length,m", po::value<size_t>(&min_length), "Minimum length of assembled sequence")
            //("orig-faa,a", po::value<std::string>(&orig_faa), "SFA-SPA assembly output before post processing (spa.fasta)")
            //("post-faa,p", po::value<std::string>(&post_faa), "Output after post processing (post.fasta)")
            ("read-bin,r", po::value<std::string>(&read_bin), "Read placement file (spa.place.bin)")
            ("call-orf,g", po::value<std::string>(&call_orf), "Gene calling file of SPA output (rtran.[fgs or mga].post)")
            //("out-dir,o", po::value<std::string>(&out_dir)->default_value("."), "Output directory")
            ("verbose,V", "Verbose");
        ...
        if ( ! vm.count("num-read") ||
             ! vm.count("min-length") ||
             //! vm.count("orig-faa") ||
             //! vm.count("post-faa") ||
             ! vm.count("read-bin") ||
             ! vm.count("call-orf") ) { 
            std::cerr << "[Error] One or more files are missing\n";
            std::cout << "\n" << USAGE_POST_STAT << "\n\n";
            std::cout << visible;
            exit(EXIT_FAILURE);
        }
