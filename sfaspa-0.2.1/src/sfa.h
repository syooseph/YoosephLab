...
// For now, only integer is allowed.
//#if BIGINT == 0
//typedef saidx_t   SfaType; 
//#else
...
typedef std::pair<SfaType, SfaType> BoundType; ///< SFA search boundary

const char DELIMITER = '$'; ///< sequence delimiter
...
class SFA
{
 protected:
	...
	SfaType       *SA;      ///< suffix array
	LcpType       *LCP;     ///< longest common prefix array
	LcpType       *mLCP;    ///< internal LCPs
	size_t        size;     ///< size of suffix array
	char          **seqs;   ///< sequence reads
	int           nreads;   ///< # of reads
	IntervalArray ia;       ///< internal LCP array
...
//=========================
 private:
	...
	/** Find left match boundary of query sequence. */
	SfaType getLeftBoundOnSFA(const SfaChar* pat, int len);

	/** Find right match boundary of query sequence. */
	SfaType getRightBoundOnSFA(const SfaChar* pat, int len);
...
//===========================
 protected:
	...
	SfaType getMcp( size_t left, size_t right );
...
//========================
 public:
	 ...
	 */
	void search( std::vector<int> &pos,
                  const SfaChar *srch, 
                  int len );
