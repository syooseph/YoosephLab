...
std::string Codon::translate(std::string &dna_str)
{
    ...
    for ( size_t i = 0; i < dna_str.length(); i += 3 ) {
        ...
        if ( dna_str.substr(i, 3) == std::string(3,'-') ) 
            aa_str += '-';
        else if ( table.find(dna_str.substr(i, 3)) == table.end() ) 
            aa_str += 'X';
        else
            aa_str += table[dna_str.substr(i, 3)];
    }
    return aa_str;
}
