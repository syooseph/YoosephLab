...
//=========================
 private:
	...
	BoundType searchOnGSA( const SfaChar *srch, int len );
	...
	SfaType getLeftBoundOnGSA(const SfaChar* pat, int len);

	/** Get right boundary of query sequence match without LCP arrays */
	SfaType getRightBoundOnGSA(const SfaChar* pat, int len);
...
//=========================
 public:
  ...
  /** Range search of given string using LCP arrays */
	...
	char* getSuffix_explicit(const unsigned int rid, const unsigned int pos);
	char* getSequence_explicit(const unsigned int rid);
	SfaType  getSuffixLength(size_t p);
	SfaType  getFullSequenceLength(size_t p);

	SfaType getId(const size_t p);
  LcpType getPos(const size_t p);	
  LcpType getLcpArbitrary(size_t p, size_t q);
  
  BoundType searchWithLCPs_bounded(const SfaChar *srch, int len, int left_bound, int right_bound);
	SfaType getLeftBoundWithLCPs_bounded(const SfaChar* pat, int len, int left_bound, int right_bound);
	SfaType getRightBoundWithLCPs_bounded(const SfaChar* pat, int len, int left_bound, int right_bound);
