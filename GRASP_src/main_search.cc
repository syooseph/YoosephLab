...
void InitDefaultParameters()  {
  ...
  input_parameters.dropoff = 25;
  input_parameters.write_certificate = false;
  return;
}

void PrintUsage() {
  std::cout << "Usage: Search query_file short_peptide_file" << std::endl;
...
int main(int argc, char** argv)	{
  ...
  desc.add_options()
      ...
      ("result_space", boost::program_options::value<std::string>(&result_directory)->default_value("Results"), "directory for writting outputs")
      ...
      ("dropoff", boost::program_options::value<unsigned int>(&input_parameters.dropoff)->default_value(25), "drop-off bit score to terminate the extension")
      ("num_threads", boost::program_options::value<unsigned int>(&input_parameters.num_threads)->default_value(1), "number of threads")
      ("write_certificate", boost::program_options::value<bool>(&input_parameters.write_certificate)->default_value(false), "whether output assembled paths and alignments")
  ;
  ...
  if(vm.count("help"))  {
    std::cout << "Usage: Search query_file short_peptide_file\n" << std::endl;
  ...
  GuidedAssemble align_and_assemble(
      working_directory, result_directory, query_file, reads_mfasta_file, input_parameters
  );
  align_and_assemble.PrepareRunEnvironment();
  
  //return 0;  
  //align_and_assemble.PreFilterAll();
  ...
  //  std::cout << static_cast<int>(it->first) << "  " << it->second.best_evalue << " " << it->second.query_seq_ID << std::endl;
